...
def get_imgs_from_autoencoder_ckpt(
    ckpt_path: str | Path,
    n=9,
    bs=15,
    device="cuda",
    ...
    for i in range(0, len(latent_tensor), bs):
        ...
        with torch.no_grad():
            im_reconstructed = model.decoder(latent_vecs).cpu().numpy()
...
def visualise_reconstructions(ckpt_dir, img_save_path, n=9):
    ...
    reconstructions = get_imgs_from_autoencoder_ckpt(ckpt_path, n=n)
...
def visualise_loss(ckpt_dir, img_save_path=None):
    ...
    plt.yscale("log")
    plt.title("Average Loss (MSE) Against Epoch")
...
def visualise_combined(ckpt_dir, save_path, n=9, ncols=3, cmap="gray", dpi=300):
    ...
    ax_loss.set_title("Average Loss (MSE)")
